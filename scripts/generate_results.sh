...
do
    ...
    MIN_MAX=$(awk 'BEGIN {min=100000} {$2 = gensub("A","","g",$2);} $2 > 0 {sum+=$2; x+=1} $2 > max {max=$2} $2 < min {min=$2} END {print min, max}' $LOG_FILE)
    MEDIAN=$(awk '{print $2+0}' $LOG_FILE | sort -n -k1 | awk 'NR==25000 {print $1}')
    echo ${COUNTER} ${MEDIAN} ${MIN_MAX} 0.5 ${F}
